/*first function to get url and seprate value on it*/
        function getUrlVars() {
            var vars = {};
...
//**************************************************************************************************************
        ...
        function result_exam()

                {
                    ...
                    var fal="corrrrrrrr";
                    var result_contact;
                    result_contact = document.getElementById("divExam");
                    for (var i = 0; i < arr.length; i++)
                    {

                        Correct_Answer=exam.Questions[arr[i]].CorrectAnswer;
                        User_Answer=exam.Questions[arr[i]].choice;
                        var HTMLtxt = '<tr>'
                        HTMLtxt+='<td>'+ exam.Questions[arr[i]].QuestionBody+'</td>'
                        HTMLtxt+='<td>'+ exam.Questions[arr[i]].CorrectAnswer+'</td>'
                        HTMLtxt+='<td>'+ exam.Questions[arr[i]].choice+'</td>'
        ...
        function timeCount()
        {
            ...
            if(min == 0 && sec == 0)
            {
                ...
                if(loadNum == 1) /*if first exam finished */
                {
                    ...
                    location.href = location.href + '?loadNum=2';
                }, 10000);
                }
                else /*if second exam finished*/
                {
                    //swal("Oops...", "Time Is Out! ... Please wait for your result", "error");
                    swal
                        (
                            {
                                  title: "Oops...",
                            ...
                            }, function()
                            {
                              ...
                              swal("Done!",
                              "Please wait for your result in about 10 sec.",
                              "success");
                            }
                        );
                    result_exam();
            ...
            console.log(end)
                ...
                $("#finish").click(function(event)  // button last
                {
                    ...
                    else /*if second exam finished*/
                    {
                        ...
                        //swal({   title: "Done!",   text: "Please wait for your result",   imageUrl: "images/thumbs-up.jpg" });
...
/*************************************** // marked questions function designed and created by Salama**********************************************************/
    ...
    function flig(index)
                {
                    //var id_input=$("#"+(index+1)+"input[name="+index+"]").prop("id");
                    var id_input=$('#_'+(index+1)+' '+'input[name="'+index+'"]').prop("id");
                    //getId=$(":radio:checked").prop("id");
                    // alert("sdfdsfsdfsdfs");
                    // alert(id_input);
                    // var z=$('input').parent();
                    // var xx=$('ff:input').prop('id');
                    // alert(xx);
                    // console.log(z.id);
                    //$("#w").remove();
                    $('#_'+(index+1)).remove();
            ...
            function mark(index)
                {
                    ...
                    $('#_2_'+(index+1)).remove();
                    drawQuset(index,id_input);
                    /*var id_input=$("#QuesMarkHover:input[name="+index+"]").prop('id');
                    alert("go to qustion");
                    alert(id_input);
                    var z=$('input').parent();
                    var xx=$('ff:input').prop('id');
                    alert(xx);
                    console.log(z.id);
                    $("#QuesMarkHover").remove();
                    alert(z.id);
                    drawMark(index,id_input);
                    */
                }
